...
<div class="w-full bg-gray-50 min-h-screen px-10 py-32">
    ...
    <div class="flex justify-center gap-3">
        <form action="{{ route('paket.update', ['id' => $paket->id]) }}" method="POST" enctype="multipart/form-data" class="w-full max-w-xl">
            ...
            <div class="mb-4">
                <label for="mc_id" class="block mb-2 text-sm font-medium text-gray-900">MC</label>
                <select name="mc_id" id="mc_id" class="bg-gray-50 border border-gray-500 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                    ...
                    @foreach ($mcs as $mc)
                    <option value="{{ $mc->id }}" {{ $mc->id == $paket->mc_id ? 'selected' : '' }}>{{ $mc->nama }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
            ...
            <div>
                <button type="submit" class="inline-block px-4 py-3 bg-amber-400 mb-4 hover:bg-amber-300 rounded-lg w-full text-sm text-white">Simpan</button>
            </div>
            <div class="mt-4 flex justify-center items-center">
                <a href="{{ route('paket') }}" class="px-4 py-3 bg-amber-400 rounded-lg hover:bg-amber-300 w-full text-sm text-center text-white hover:text-black hover:font-bold hover:shadow-md">Kembali</a>
            </div>
        </form>
    </div>
</div>
